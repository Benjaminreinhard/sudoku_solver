...
def render_states(state_infos, coloring_enabled):
	...
	color = lambda label : font_style(label, [GREEN, BOLD], coloring_enabled)
...
def render_counts_of_available_steps(trick_order, outline, num_of_entries_per_row):
	...
	entry_width = 4
	format_entry = lambda x : fill_to_width(str(x) if x > 0 else '.', 4)
	...
	format_row = lambda label, l: f'{fill_to_width(label, sidebar_width)}| {''.join(l)}'

	horizontal_line = lambda n : f'{'-' * sidebar_width}+-{'-' * n * entry_width}'

	num_of_counts = len(formatted_counts['total'])

	ceil = lambda x : int(x) if x - int(x) == 0 else int(x) + 1
